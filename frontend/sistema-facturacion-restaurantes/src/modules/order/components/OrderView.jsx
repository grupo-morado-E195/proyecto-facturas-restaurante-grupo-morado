...
export default function OrderView({ orden, onClose }) {
  ...
  return (
    <Modal
      title={`Detalle Orden #${String(ordenId).padStart(3, "0")}`}
      onClose={onClose}
    >
      <div className="bg-gray-50 rounded-lg px-4 py-3 mb-4 text-sm flex justify-between items-center border border-gray-100">
        <div>
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden mb-4 bg-white">
        <div className="grid grid-cols-[1fr_auto_100px] gap-2 px-3 py-2 bg-gray-50 border-b border-gray-200">
        ...
        {detalles.length === 0 ? (
          <p className="text-center text-gray-400 py-4 text-xs px-3">
          ...
          detalles.map((d, i) => (
            <div
              key={i}
              className=" px-3 py-2.5 border-b border-gray-100 last:border-0 hover:bg-gray-50/50 transition-colors"
            >
              <div className="grid grid-cols-[1fr_auto_100px] gap-2 items-center">
              ...
              {d.observaciones && (
                <div className="flex items-center gap-1 mt-1 text-[11px] text-amber-700 bg-amber-50 px-2 py-0.5 rounded border border-amber-100 w-fit">
      ...
      {subtotal !== undefined && (
        <div className="flex justify-between font-black text-sm border-t border-gray-200 pt-3 flex-col gap-1.5 bg-gray-50/30 p-3 rounded-lg border border-gray-100">
          ...
          <div className="flex justify-between text-[#E87722] font-bold text-sm border-t border-gray-200/60 pt-2 mt-0.5">
            <span>Total</span>
            <span>{fmt(total)}</span>
          </div>
        </div>
